...
const StatementOfAccount = () => {
  ...
  const { data: soaData, loading: soaLoading, error: soaError } = useAxios({
  ...
  React.useEffect(() => {
    if (soaData) {
      const filteredData = soaData
        .map((item: StatementOfAccountDataType) => ({
          ...item,
  ...
  if (soaLoading) return <Typography variant="h4" color="initial">Loading...</Typography>
  if(soaError) return <Typography variant="h4" color="initial">{soaError}</Typography>
